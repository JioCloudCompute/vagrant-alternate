...
def SANITIZE(ignore_arg):
	# Remove all /tmp/%.monitor.sock
	# Empty the dnsmasq lease and /tmp/puppet-hosts
        # clean image folder
        os.system("sudo bash -l -c %s/sanitize.sh" % os.path.dirname(os.path.realpath(__file__)))
